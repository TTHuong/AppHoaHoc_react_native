...
export default class Baitap extends Component {
    ...
    loadDataBaiTap(kt,Id,Mon){
        ...
        if(kt){
            firebaseApp.database().ref("BaiTap").child(Id).orderByChild('idbaitap').limitToFirst(4).once('value',function(snapshot){
                let array = [];
                snapshot.forEach(function(childSnapshot) {
                    var childData = childSnapshot.val();
                    lasttolist=childSnapshot.val().idbaitap;
                    // console.log("cai dau "+lasttolist+ " tieu de "+childData.tieude );
                    array.push({
                        idbaitap : childData.idbaitap,
            ...
            firebaseApp.database().ref("BaiTap").child(Id).orderByChild('idbaitap').limitToLast(1).once('value',function(snapshot){
                snapshot.forEach(function(childSnapshot) {
                   that.setState({
                       vtcuoi:childSnapshot.val().idbaitap,
                   })
            ...
            {
                firebaseApp.database().ref("BaiTap").child(Id).orderByChild('idbaitap')
            ...
            <FlatList
                    ...
                    onEndReached={()=>{
                        if(this.state.actitimkiem==false){
                            this.loadDataBaiTap(false,this.state.IdMonhoc,this.state.Mon);
                        }
                    }}
                                        ...
                                        <View style={Styles.dkdn_khungbangchonlop_middle_khung}>
                                            <FlatList 
                                                ...
                                                renderItem={({item})=>
                                                    item.lop==this.state.Lop?
                                                        <View style={Styles.home_khungmonhocnho}>
                                                            <TouchableOpacity style={Styles.home_khunganhmonhocnho} onPress={()=>this.loadDataBaiTap(true,item.id,item.tenmon)}>
                                                                <Image source={{uri:item.anh}} style={Styles.home_anhmonhocnho} />
                                                            </TouchableOpacity>
                                                                <Text style={Styles.home_chumonhocnho}>{item.tenmon}</Text>
                                                        </View>
